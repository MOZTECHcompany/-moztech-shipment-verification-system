...
function requestPerformance(pool) {
    ...
    return (req, res, next) => {
        ...
        // Express strips mount prefixes while a nested router sends a response.
        // Capture the original API path before that mutation; never retain query data.
        ...
        res.once('finish', () => {
            ...
            const metric = { event: 'wms_request_perf', route: req.route?.path || requestPath.replace(/\/[0-9]+(?=\/|$)/g, '/:id'), method: req.method,
