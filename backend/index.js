...
app.put('/api/admin/users/:userId', authenticateToken, authorizeAdmin, async (req, res) => {
    ...
    if (Number(userId) === req.user.id && role && role !== 'admin') return res.status(400).json({ message: '无法修改自己的管理员权限' });
    try {
...
app.post('/api/orders/import', authenticateToken, upload.single('orderFile'), async (req, res) => {
    ...
    try {
        ...
        let itemsStartRow = -1;
        for(let i = 0; i < data.length; i++) { if (data[i] && data[i].includes('品项编码')) { itemsStartRow = i + 1; break; } }
        if (itemsStartRow === -1) return res.status(400).json({ message: 'Excel 档案中找不到 "品项编码" 标题行' });
        const itemsData = data.slice(itemsStartRow).filter(row => row[0] && row[2]);
        if (!voucherNumber) return res.status(400).json({ message: 'Excel 档案缺少单据编号 (请检查 A2 储存格是否为 "凭证号码：...")' });
        await client.query('BEGIN');
        const existingOrder = await client.query('SELECT id FROM orders WHERE voucher_number = $1', [voucherNumber]);
        if (existingOrder.rows.length > 0) { await client.query('ROLLBACK'); return res.status(409).json({ message: `订单 ${voucherNumber} 已存在，请勿重复汇入` }); }
        ...
        for (const row of itemsData) {
            const productCode = row[0]; const productName = row[1]; const quantity = parseInt(row[2], 10);
        ...
        await client.query('ROLLBACK');
        console.error('汇入订单失败:', err);
        res.status(500).json({ message: '处理 Excel 档案时发生错误，请检查格式或联络管理员' });
    } finally {
        client.release();
    }
});
...
app.post('/api/orders/update_item', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (itemResult.rows.length === 0) { await client.query('ROLLBACK'); return res.status(404).json({ message: '在订单中找不到此产品' }); }
        const item = itemResult.rows[0];
        const order = (await client.query('SELECT * FROM orders WHERE id = $1', [orderId])).rows[0];
        if (type === 'pick' && order.picker_id !== userId && order.status === 'picking') { await client.query('ROLLBACK'); return res.status(403).json({ message: '您不是此订单的指定拣货员' }); }
        if (type === 'pack' && order.packer_id !== userId && order.status === 'packing') { await client.query('ROLLBACK'); return res.status(403).json({ message: '您不是此订单的指定装箱员' }); }
        let newPickedQty = item.picked_quantity;
        let newPackedQty = item.packed_quantity;
        if (type === 'pick') {
            newPickedQty += amount;
            if (newPickedQty < 0 || newPickedQty > item.quantity) { await client.query('ROLLBACK'); return res.status(400).json({ message: '拣货数量无效' }); }
            await client.query('UPDATE order_items SET picked_quantity = $1 WHERE id = $2', [newPickedQty, item.id]);
        } else if (type === 'pack') {
            newPackedQty += amount;
            if (newPackedQty < 0 || newPackedQty > item.picked_quantity) { await client.query('ROLLBACK'); return res.status(400).json({ message: '装箱数量不能超过已拣货数量' }); }
