...
- 原附件 ID 3、4（各 47,300 bytes）再次授權下載仍 404，已查的原 checkout 也沒有副本。保留完整 metadata，尚未取得內容，不算遷移完成。
...
| 工作區 UI | 本機瀏覽器走通：picker 登入 → 開始揀貨 → 3 次掃碼 → packer 接手 → 3 次掃碼 → 完成列表。設定齒輪可見，倉儲員看不到成員管理。 |
...
6. 追加的 v3 管理員瀏覽器驗收遇到 Mac 鎖定、點擊沒有改變頁面，尚未完成；先前本機三件商品揀貨／裝箱流程已通過，不能用 API 測試取代現場操作。
