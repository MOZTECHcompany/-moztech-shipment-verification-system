...
- 保留 Web Speech、語音開關及個人掃碼音效。完成播報期間，新任務通知不會中斷它；其後掃碼僅保留最新一則，避免排隊播報過時數量。音訊裝置失敗不影響伺服器已確認的掃码結果。
...
- 本次提供的 Excel 問題出於 `理貨單!A13`：底層數字完整，但 General 顯示成科學記號。攔下並提供檔案儲存值供核對，不猜碼或自动套用。原始工作簿未修改，也未寫入 Git。
...
發版前基準：GitHub `main` 為 `32e697ad763ccf7eab26eb9d2600979e3c5b908c`；正式 `corely-wms-chat-time-20260916` 接收 100%。本次候選與正式結果完成後補錄。
