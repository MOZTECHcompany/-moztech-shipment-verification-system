...
| 3290494 | 後台與正式 API 已驗證 | 7-ELEVEN 常溫、全家、萊爾富 B2C；金鑰與權限已核對 |
...
7. 實體印表機、現場收退貨流程驗收；雙帳號查詢 UI 與綠界託運單預覽已在第二次候選完成。
...
## 第二帳號補驗（2026-09-10 17:02 之後）

- 已登入商店 3290494（MOZTECH墨子科技／萬博創意科技有限公司加開商代），B2C 常溫三通路測標通過，C2C 未啟用；退貨通／退貨便已開通。
- 近一週未封存物流查詢 104 筆，並確認 3 件買家未取、已退回物流中心：49896073、49857793、49838240。這是綠界物流中心，不能當作公司倉庫已收貨。
- 付款與物流 HashKey / IV 相同經後台核對；既有 ERP profile 的簽章查詢亦成功。新增 WMS 專用 Secret version 2，沒有修改 ERP Secret 或綠界後台設定。
- 支援既有 Shopify 格式 `#152746` 的廠商單號查詢，已由正式 API 驗證。只允許單一前置 # 與英數字，總長度上限 20；原值簽章與送出，不剝除 #。
- 補齊實查貨態：7-ELEVEN / 全家 310＝訂單上傳物流中；7-ELEVEN 2076＝未取件、已退回物流中心。其他未驗證的通路／代碼維持未對應，不推論已送達或實收。
- 2076 會建立一筆去重的預期退回，保留未關聯 WMS 訂單狀態，沒有退款、入庫或銷貨退回。
- 新增 3 項回歸測試，合計 20 項通過；前端建置通過。
- 第一帳號已完成瀏覽器登入→設定→物流串接→查詢，切換帳號清除結果，以及從 WMS 產生綠界官方託運單预覽；沒有送到實體印表機。
- 雙帳號建置 88eedf24-6b5f-4432-93c6-eb667e0794ef；來源摘要 7a446a6b26d9。候選部署與最終驗收結果見 artifacts/wms-ecpay-second-20260910。先前段落是第一次候選驗收快照。

最終結果：候選 `corely-wms-00009-ldj` 已通過雙帳號 API 與瀏覽器驗收，使用 Secret version 2；正式 `corely-wms-00010-hug` 仍為 100%。第二帳號已加入三筆未取退回物流中心記錄與既有出貨單 50124899，並成功產生官方託運單預覽。未執行實體列印、全量匯入、自動追蹤或 ERP 銷貨退回。物流頁頂部頁名仍沿用「工作台」，留待後續導覽文案整理。
