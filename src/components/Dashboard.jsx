...
export function Dashboard({ user, onLogout }) {
  ...
  useEffect(() => { if (orderHeader) localStorage.setItem(KEY_ORDER_HEADER, JSON.stringify(orderHeader)); else localStorage.removeItem(KEY_ORDER_HEADER); }, [orderHeader]);
  ...
  const handleExcelImport = async (e) => {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      // 使用 apiClient，它會自動加上 baseURL 和 Authorization token
            ...
            {sortedShipmentData.length > 0 ? (
              <div className="space-y-3">
                {sortedShipmentData.map((item) => {
                  ...
                  return (
                    <div key={item.sku} className={`border rounded-lg p-4 flex flex-col sm:flex-row items-center justify-between gap-4 transition-all`}>
