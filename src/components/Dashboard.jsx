...
export function Dashboard({ user, onLogout }) {
  ...
  // (此處省略 handleLogout, handleExportReport 等其他不變的函數)
  // ...
  const handleLogout = () => { localStorage.clear(); onLogout(); };
  const handleScan = () => { /* 掃描邏輯... */ }; // (省略以求簡潔)
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto bg-gray-50 min-h-screen">
      {/* (省略 Header, ProgressDashboard 等不變的 JSX) */}
      <header>...</header>
      <ProgressDashboard stats={progressStats} onExport={() => {}} />
            ...
            {sortedShipmentData.length > 0 ? (
              <div className="space-y-3">
                {sortedShipmentData.map((item) => {
                  ...
                  const status = getItemStatus(item, packedQty, packedQty); // 簡化
                  return (
                    <div key={item.sku} className={`border rounded-lg p-4 flex items-center justify-between gap-4 transition-all`}>
