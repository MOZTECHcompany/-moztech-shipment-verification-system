...
export function UserManagement({ currentUser }) {
    ...
    const roleMap = { 
        superadmin: { label: '最高管理員', variant: 'danger', icon: Shield },
        admin: { label: '管理員', variant: 'warning', icon: Shield },
        dispatcher: { label: '拋單員', variant: 'info', icon: UserIcon },
        picker: { label: '揀貨員', variant: 'purple', icon: UserIcon },
        packer: { label: '裝箱員', variant: 'success', icon: UserIcon }
    };
                                    ...
                                    <TBody>
                                        {users.map((user) => (
                                            <TR key={user.id}>
                                                ...
                                                <TD>
                                                    <Badge
                                                        variant={roleMap[(user.role || '').toLowerCase()]?.variant || 'neutral'}
                                                        className="inline-flex items-center gap-1"
                                                    >
                                                        {((user.role || '').toLowerCase() === 'admin' || (user.role || '').toLowerCase() === 'superadmin') && <Shield className="h-3 w-3" />}
