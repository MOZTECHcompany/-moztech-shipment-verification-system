            ...
            return <div key={type} className="space-y-1">
                <label className="block text-sm text-slate-700">{label}語音
                    <select aria-label={`${label}語音`} disabled={!settings.supported} value={settings.selected[type]} onChange={event => update(() => voiceNotification.setStageVoice(type, event.target.value))} className="mt-1 min-h-11 w-full min-w-0 rounded-lg border border-slate-300 bg-white px-2 text-slate-900">
                        ...
                        {missing && <option value={settings.selected[type]}>原語音不可用{settings[type].voice ? '（改用台灣中文）' : ''}</option>}
                        {settings.voices.map(voice => <option key={voiceId(voice)} value={voiceId(voice)}>{voice.name} · {voice.lang}</option>)}
                    </select>
                </label>
                <div className="flex items-center justify-between gap-2"><span className="min-w-0 break-words text-xs text-slate-500">{settings[type].voice?.name || '沒有可用的台灣中文語音'}</span>
                    <button type="button" disabled={!settings.supported || !settings[type].voice} onClick={() => { if (!voiceNotification.preview(type)) toast.info('目前無法播報，請檢查裝置音量與台灣中文語音。'); }} className="min-h-11 shrink-0 rounded-lg px-3 text-sm text-blue-700 disabled:opacity-40">試聽{label}</button>
                </div>
            </div>;
        })}
        <p className="text-xs leading-5 text-slate-500">{!settings.supported ? '此瀏覽器不支援語音播報。' : !settings.voices.length ? '請在裝置安裝台灣中文語音後重新整理。目前可用個人掃碼音效提示。' : settings.sameVoice ? '目前揀貨與裝箱使用同一語音，不變調。可搭配不同掃碼音效區分，或安裝另一種台灣中文男女聲。' : '使用台灣中文原聲，請先試聽確認。設定套用於此瀏覽器。'}</p>
    </div>;
    return compact ? <details className="mb-3 rounded-lg border border-slate-200 bg-white px-3 py-2"><summary className="min-h-8 cursor-pointer text-sm text-slate-600">語音播報：{settings.enabled ? settings.voices.length ? '已開啟' : '缺少台灣中文語音' : '已關閉'}</summary>{controls}</details>
