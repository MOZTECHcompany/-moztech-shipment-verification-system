// src/components/Dashboard.jsx (最终修正版)
import React, { useState, useRef, useEffect, useMemo, useCallback } from 'react';
import * as XLSX from 'xlsx';
import { toast } from 'sonner';
import apiClient from '@/api/api.js'; // ✨✨✨ 最终的、正确的路径修正 ✨✨✨
...
export function Dashboard({ user, onLogout }) {
  ...
  const handleExportReport = () => { /* ... (Your existing logic) ... */ };
  const handleQuantityChange = (sku, type, amount) => { /* ... (Your existing logic) ... */ };
  const triggerFlash = (sku, type) => { setFlash({ sku, type }); setTimeout(() => setFlash({ sku: null, type: null }), 700); };
  const playSound = (type) => { /* ... (Your existing logic) ... */ };
  const handleError = (errorData) => { /* ... (Your existing logic) ... */ };
  const handleScan = () => { /* ... (Your existing logic) ... */ };
