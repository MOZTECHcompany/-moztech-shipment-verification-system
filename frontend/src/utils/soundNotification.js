...
class SoundNotification {
    ...
    // Picking: one short high tone. Packing: two lower, rising tones.
    async playScanSuccess(type) {
        if (!this.enabled) return;
        try {
            if (type === 'pick') {
                await this.playTone(1200, 0.08, 0);
            } else {
                await this.playTone(660, 0.08, 0);
                await this.playTone(880, 0.1, 0.1);
            }
        } catch (err) {
            console.warn('掃描提示音播放失敗:', err);
        }
    }
    ...
    play(soundName) {
        ...
        const methods = {
            ...
            pickSuccess: () => this.playScanSuccess('pick'),
            packSuccess: () => this.playScanSuccess('pack'),
